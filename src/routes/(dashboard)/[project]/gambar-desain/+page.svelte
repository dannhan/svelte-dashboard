...
<div
  class="mx-auto grid max-w-[948px] grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-1 md:gap-6 lg:grid-cols-2"
>
  ...
  {#each imageURLs as src}
    ...
    >
      ...
      <button
        class="absolute left-0 top-0 z-50 h-full w-full bg-transparent text-black opacity-0"
        on:click={() => document.getElementById(src)?.requestFullscreen()}
      >
      </button></AspectRatio
    >
  {/each}
